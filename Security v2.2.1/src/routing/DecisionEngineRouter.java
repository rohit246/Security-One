...
/**
	...
	public void testmalicious(Message m, DTNHost otherHost, DTNHost from){
		...
		for(int z=0; z<=otherHost.nodecount-1;z++){
			...
			if((otherHost.LT.get(local_host)!=null) && (otherHost.LT.get(local_T)!=null)){
				...
				if(bum_block==block){
					...
					testing.addEdge(otherHost.toString(),otherHost.connected_nodes[z].toString(),block);
					...
					if(bum_block>block){
						...
						testing.addEdge(otherHost.toString(),otherHost.connected_nodes[z].toString(),enc_time);
		...
		for(int z=0; z<=otherHost.nodecount-1;z++){
			...
			if(testing.get_t(otherHost.toString(),otherHost.connected_nodes[z].toString())!=0){
				//enc_time2 = host1.LER.get(otherHost.toString(),otherHost.connected_nodes[z].toString());
				enc_time2 = testing.get_t(otherHost.toString(),otherHost.connected_nodes[z].toString());
			}
			...
			if(otherHost.LT.get(ot_host)!=null){
				...
				if(enc_time2<=wiki){
					...
					testing.addEdge(otherHost.toString(),otherHost.connected_nodes[z].toString(),wiki);
					for(i = 0; i<otherHost.nodecount-1; i++){
		...
		*/
		...
		if( thistime > 3000/*flag1==1 || flag2==1 || flag3==1 || flag4==1*/){
			for(i=0;i<allNodes.size();i++){
				...
				for(j=0;j<allNodes.size();j++){
					if(i!=j){
						...
						if(testing.get_t(allNodes.get(i).toString(),allNodes.get(j).toString())!=0){
							block = testing.get_t(allNodes.get(i).toString(),allNodes.get(j).toString());
						...
						if(f_tau_calc>dec_tau && testing.get_t(allNodes.get(i).toString(),allNodes.get(j).toString())!=0){	
							otherHost.modmalicious=1;
		...
		System.out.println("urrrLT"+host1.msg_overhead);
		host1.msg_overhead+=testing.get_size();
		System.out.println("urrrtest"+host1.msg_overhead);
