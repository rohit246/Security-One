...
public class SnFDecisionEngine implements RoutingDecisionEngine
{
	...
	public SnFDecisionEngine replicate()
	{
	...
	{
		if(!peer.modname.startsWith("IBN_")){		
	...
	public void updatetable(Message m, DTNHost otherHost, DTNHost myHost, int nrofCopies)
	{
			MessageInfo mytemp = new MessageInfo(m.getId(), m, otherHost.toString(), m.getTtl(), m.size, nrofCopies/2, 1);
			MessageInfo othertemp = new MessageInfo(m.getId(), m, myHost.toString(), m.getTtl(), m.size, nrofCopies/2, 1);
			myHost.spray_table.add(mytemp);
			otherHost.spray_table.add(othertemp);
			
			/***
			myHost.rowcount = myHost.rowcount + 1;
			otherHost.rowcount = otherHost.rowcount + 1;
			myHost.spray_table_ID[myHost.rowcount] = m.getId();
			myHost.spray_table_msg[myHost.rowcount] = m;
			myHost.spray_table_msgsize[myHost.rowcount] = m.size;
			myHost.spray_table_other[myHost.rowcount]= otherHost.modname;
			myHost.spray_table_TTL[myHost.rowcount] = m.getTtl();
			myHost.spray_table_noms[myHost.rowcount] = nrofCopies/2;
			myHost.spray_table_nomr[myHost.rowcount] = 1;
			otherHost.spray_table_ID[otherHost.rowcount] = m.getId();
			otherHost.spray_table_msg[otherHost.rowcount] = m;
			otherHost.spray_table_msgsize[otherHost.rowcount] = m.size;
			otherHost.spray_table_other[otherHost.rowcount] = myHost.modname;
			otherHost.spray_table_TTL[otherHost.rowcount] = m.getTtl();
			otherHost.spray_table_noms[otherHost.rowcount] = nrofCopies/2;
			otherHost.spray_table_nomr[otherHost.rowcount] = 1;
			***/
	...
	{
		if(otherHost.mal_cnt < 3){
			if(m.getTo() == otherHost){
				updatetable(m,otherHost,myHost,0);
				return true;
			}
			
			int nrofCopies = (Integer)m.getProperty(MSG_COUNT_PROP);
			if(nrofCopies > 1){
				updatetable(m,otherHost,myHost,nrofCopies);
				return true;
			}
