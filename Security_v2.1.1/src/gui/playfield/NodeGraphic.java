...
public class NodeGraphic extends PlayFieldGraphic {
	...
	private static Color range1Color = Color.YELLOW;
	private static Color conColor = Color.BLACK;
	private static Color con1Color = Color.CYAN;
	private static Color hostColor = Color.BLUE;
	private static Color smalhostColor = Color.RED;
	private static Color fmalhostColor = Color.BLACK;
	...
	private void drawHost(Graphics2D g2) {
		...
		if (drawConnections) {
			if(node.modname.startsWith("SAT_")){
				g2.setColor(con1Color);
			}
			else{
		...
		else if(node.malnode_spray == 1)
			g2.setColor(smalhostColor);
		else if(node.malnode_focus == 1)
			g2.setColor(fmalhostColor);
		else
