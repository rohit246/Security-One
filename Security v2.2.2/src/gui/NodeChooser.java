...
public class NodeChooser extends JPanel implements ActionListener {
	...
	private void setNodes(int offset) {
		...
		for (int i=offset; i<nodes.size() && i < offset + MAX_NODE_COUNT; i++) {
			...
			if(h.malnode_spray == 1){
				JButton jb = new JButton(h.toString() + " sm");
				jb.putClientProperty(HOST_KEY, h);
				jb.addActionListener(this);
				nodesPanel.add(jb);
			}
			else if(h.malnode_focus == 1){
				JButton jb = new JButton(h.toString() + " fm");
				jb.putClientProperty(HOST_KEY, h);
				jb.addActionListener(this);
				nodesPanel.add(jb);
			}
			else if(h.ferry == 1){
				JButton jb = new JButton(h.toString() + " ferry");
				jb.putClientProperty(HOST_KEY, h);
				jb.addActionListener(this);
				nodesPanel.add(jb);
			}
