 ...
 */
public class AllMaliciousNodesReport extends Report 
	...
	private void processEvent(final String action, final DTNHost host1, 
		...
		if(host1 != null && host1.malchk == 0){
			if(host1.malnode_spray == 1){
				host1.malchk = 1;
				write(host1 + " is spray malicious");
			}
			else if(host1.malnode_focus == 1){
				host1.malchk = 1;
				write(host1 + " is focus malicious");
			}
			else if(host1.ferry == 1){
				host1.malchk = 1;
				write(host1 + " is ferry");
			}
		}
		if(host2 != null && host2.malchk == 0){
			if(host2.malnode_spray == 1){
				host2.malchk = 1;
				write(host2 + " is spray malicious");
			}
			else if(host2.malnode_focus == 1){
				host2.malchk = 1;
				write(host2 + " is focus malicious");
			}
			else if(host2.ferry == 1){
				host2.malchk = 1;
				write(host2 + " is ferry");
			}
		}
	}
	...
	/*	String extra;
		...
		processEvent(StandardEventsReader.DELIVERED, from, to, m, null);
	}
