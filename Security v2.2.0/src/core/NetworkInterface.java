...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	protected void connect(Connection con, NetworkInterface anotherInterface) {
		...
		notifyConnectionListeners(CON_UP, anotherInterface.getHost(), con);
		...
		this.host.conup(anotherInterface.getHost());
	...
	/*protected void disconnect(Connection con, 
		...
		notifyConnectionListeners(CON_DOWN, anotherInterface.getHost(),con);
	...
	{
		notifyConnectionListeners(CON_DOWN, anotherInterface.getHost(), con);
		this.getHost().condown(anotherInterface.getHost());
		/*SimClock time = new SimClock();
		con.contime = time.getTime() - con.conup;
		if(con.msgOnFly != null){
			con.msgOnFly.chk++;
			con.msgOnFly.transmittime = con.msgOnFly.msgsize/(this.transmitSpeed*1000);
			if(con.getRemainingByteCount() > 0 && con.msgOnFly.transmittime < con.contime && con.msgOnFly.getTtl() > 0)
				this.host.mal_cnt++; 
		}*/
	}
	 ...
	 */
	public void notifyConnectionListeners(int type, DTNHost otherHost, Connection con) {
		...
		for (ConnectionListener cl : this.cListeners) {
			...
			case CON_UP:
				...
				/*SimClock time = new SimClock();
				con.conup = time.getTime();*/
				cl.hostsConnected(this.host, otherHost, con);
				break;
			case CON_DOWN:
				/*SimClock time1 = new SimClock();
				con.contime = time1.getTime() - con.conup;
				if(con.msgOnFly != null){
					con.msgOnFly.chk++;
					con.msgOnFly.transmittime = con.msgOnFly.msgsize/(this.transmitSpeed*1000);
					if(con.getRemainingByteCount() > 0 && con.msgOnFly.transmittime < con.contime && con.msgOnFly.getTtl() > 0)
				...
				cl.hostsDisconnected(this.host, otherHost, con);
				break;
			default:
	...
	public void destroyConnection(NetworkInterface anotherInterface) {
		...
		//this.getHost().condown(anotherHost);
	...
	private void removeConnectionByIndex(int index) {
		...
		con.setUpState(false);
		notifyConnectionListeners(CON_DOWN, anotherNode, con);
