...
/**
 * Implementation of MaxPropRouter router as described in 
 * <I>MaxPropRouter: Routing for Vehicle-Based Disruption-Tolerant Networks</I> by
 * John Burgess et al.
 * @version 1.0
 * 
...
public class MaxPropRouter extends ActiveRouter {
	...
	public static final String MaxPropRouter_NS = "MaxPropRouter";
	/**
	...
	public MaxPropRouter(Settings settings) {
		super(settings);
		Settings MaxPropRouterSettings = new Settings(MaxPropRouter_NS);		
		if (MaxPropRouterSettings.contains(ALPHA_S)) {
			alpha = MaxPropRouterSettings.getDouble(ALPHA_S);
		} else {
			alpha = DEFAULT_ALPHA;
		}

        Settings mpSettings = new Settings(MaxPropRouter_NS);
	...
	public void changedConnection(Connection con) {
		if (con.isUp()) { // new connection
			...
			if (con.isInitiator(getHost())) {
				...
				if(!otherHost.modname.startsWith("IBN_")){
					def = 1;	
					assert mRouter instanceof MaxPropRouter : "MaxPropRouter only works "+ 
					...
					this.ackedMessageIds.addAll(mRouter.ackedMessageIds);
					mRouter.ackedMessageIds.addAll(this.ackedMessageIds);
					deleteAckedMessages();
					mRouter.deleteAckedMessages();
	 ...
	 * at {@link ActiveRouter#update()}. MaxPropRouter makes book keeping of the
	...
	/**
	 * Returns the next message that should be dropped, according to MaxPropRouter's
	 * message ordering scheme (see {@link MaxPropRouterTupleComparator}). 
	...
	protected Message getOldestMessage(boolean excludeMsgBeingSent) {
		...
		Collections.sort(validMessages, 
				new MaxPropRouterComparator(this.calcThreshold())); 
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		for (Connection con : getConnections()) {
			...
			if(!other.modname.startsWith("IBN_")){
		 ...
		 * defined in MaxPropRouterTupleComparator */ 
		Collections.sort(messages, new MaxPropRouterTupleComparator(calcThreshold()));
	...
	/**
	 * Message comparator for the MaxPropRouter routing module. 
	 ...
	 */
	private class MaxPropRouterComparator implements Comparator<Message> {
		 ...
		 */
		public MaxPropRouterComparator(int treshold) {
		 ...
		 */
		public MaxPropRouterComparator(int treshold, DTNHost from1, DTNHost from2) {
			this.threshold = treshold;
			this.from1 = from1;
			this.from2 = from2;
		}
	...
	/**
	 * Message-Connection tuple comparator for the MaxPropRouter routing 
	 * module. Uses {@link MaxPropRouterComparator} on the messages of the tuples
	 ...
	 * of the connection).
	 */
	private class MaxPropRouterTupleComparator 
			implements Comparator <Tuple<Message, Connection>>  {
		private int threshold;
		
		public MaxPropRouterTupleComparator(int threshold) {
			this.threshold = threshold;
		}
		
		/**
		 * Compares two message-connection tuples using the 
		 * {@link MaxPropRouterComparator#compare(Message, Message)}.
		 */
		public int compare(Tuple<Message, Connection> tuple1,
				Tuple<Message, Connection> tuple2) {
			MaxPropRouterComparator comp;
			DTNHost from1 = tuple1.getValue().getOtherNode(getHost());
			DTNHost from2 = tuple2.getValue().getOtherNode(getHost());
			
			comp = new MaxPropRouterComparator(threshold, from1, from2);
